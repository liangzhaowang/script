...
def do(path):
    ...
    if os.path.exists(dataPath):
        with open(dataPath) as dataFile:
            resultObj = json.loads("[" + dataFile.read() + "]")
            for jsonObj in resultObj:
                GenExcel(jsonObj)
    else:
        print "file %s not exists!" % (dataPath)

def GenExcel(jsonObj):
    f = xlwt.Workbook()
...
def set_style(name,height,bold=False,isSetBG=False):
    ...
    contentStyle = xlwt.easyxf('pattern: pattern solid; borders: left 1, right 1, top 1, bottom 1;')
    if isSetBG:
        return titleStyle
    else:
        return contentStyle
